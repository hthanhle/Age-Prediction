...
class FaceDataset(Dataset):
    def __init__(self, dataset_dir,
        ...
        self.transforms = T.Compose([
            T.Resize(self.input_shape, interpolation=Image.BILINEAR),
            T.RandomHorizontalFlip(p=0.5),
            T.RandomApply([T.RandomAffine(15, (0.05, 0.05), (0.95, 1.25),
                                          (0.2, 0.2, 0.2, 0.2), 2)], p=0.5),
            T.RandomApply([T.ColorJitter(brightness=0.2, contrast=(0.85, 1.0))], p=0.4),
            T.RandomPerspective(0.05, p=0.2),
            T.ToTensor(),
...
# Soan's code: try to deal with the problem of Unicode filename
