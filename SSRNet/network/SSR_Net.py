...
class SSR_Net(nn.Module):
    """
    A Compact Soft Stagewise Regression Network for Age Estimation
    """

    def __init__(self, stage_num, lambda_local, lambda_d, max_age=None):
        ...
        self.x_layer4_mix = nn.Sequential(
            nn.Dropout(p=0.2),
            nn.Linear(in_features=640, out_features=self.stage_num[0]),
            nn.ReLU(inplace=True),
        )
        ...
        self.pred_a_s1 = nn.Sequential(
            nn.Linear(in_features=2 * self.stage_num[0], out_features=self.stage_num[0]),
            nn.ReLU(inplace=True),
        )
        ...
        self.s_layer2_mix = nn.Sequential(
            nn.Dropout(p=0.2),
            nn.Linear(in_features=160, out_features=self.stage_num[1]),
            nn.ReLU(inplace=True),
        )
        # -------------------------------------------------        
        ...
        self.feat_a_s2 = nn.Sequential(
            nn.Linear(in_features=stage_num[1], out_features=2 * self.stage_num[1]),
            nn.ReLU(inplace=True),
        )
        self.pred_a_s2 = nn.Sequential(
            nn.Linear(in_features=2 * self.stage_num[1], out_features=self.stage_num[1]),
            nn.ReLU(inplace=True),
        )
        ...
        self.s_layer1_mix = nn.Sequential(
            nn.Dropout(p=0.2),
            nn.Linear(in_features=160, out_features=self.stage_num[2]),
            nn.ReLU(inplace=True),
        )
        # -------------------------------------------------        
        ...
        self.feat_a_s3 = nn.Sequential(
            nn.Linear(in_features=self.stage_num[2], out_features=2 * self.stage_num[2]),
            nn.ReLU(inplace=True),
        )
        self.pred_a_s3 = nn.Sequential(
            nn.Linear(in_features=2 * self.stage_num[2], out_features=self.stage_num[2]),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, x):
        ...
        # x_layer2->x3->x_layer3->x4 : 32*16*16->32*8*8->32*8*8
        x3 = self.x3(x_layer2)
        ...
        # s_layer2->s3->s_layer3->s4 : 16*16*16->16*8*8->16*8*8
        s3 = self.s3(s_layer2)
