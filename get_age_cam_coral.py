...
Predictions stabilized over all previous frames
"""
import argparse
...
if __name__ == '__main__':
    ...
    coral = RegressionModel(101, backbone='resnext101_32x8d').to(device)
    coral.load_state_dict(torch.load('./CORAL/checkpoints/epoch_2_mae_10.8020.pt', device))
    ...
    while True:
        ...
        for det in detections:
            ...
            if idx is None:  # if the current detection is a new one (not exist in the list)
                ...
                pre_ages.append([pred_age])  # 'pre_ages' is a list whose each element is also a list for storing all previous predicted ages
